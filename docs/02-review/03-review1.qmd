...
## [R]{style=color:blue;} showcase

**[[ggplot2]{style=color:blue;}](https://ggplot2.tidyverse.org/reference/index.html)**
...
- - -
**[[shiny]{style=color:blue;}](https://shiny.rstudio.com)**
...
$$ \color{#e64173}{y_i} = \beta_0 + \beta_1 \color{#6A5ACD}{x_{1i}} + \beta_2 \color{#6A5ACD}{x_{2i}} + \cdots + \beta\_k \color{#6A5ACD}{x_{ki}} + u_i $$
...
- Generate 10,000 observations on variables $x_1$ through $x_{1000}$ Regressions 
...
```{r, r2 data, include = F, cache = T,eval=FALSE}
set.seed(1234)
y <- rnorm(1e4)
x <- matrix(data = rnorm(1e7), nrow = 1e4)
x %<>% cbind(matrix(data = 1, nrow = 1e4, ncol = 1), x)
...
**One solution:** [Adjusted]{styple=color:pink;} $\color{#e64173}{R^2}$
