...
from the unknown population parameter. [^1]
...
[^1]: Except when we run the simulation
...
-   **Confidence intervals:** Use the estimate and its standard error to
    ...
    generally[^2] contain the true parameter.

-   **Hypothesis tests:** Determine whether there is statistically
    significant evidence to reject a hypothesized value or range of
    values.

[^2]: *Hint:* it's normal with the mean and variance
we've derived/discussed above) <br> : *E.g.*, Similarly
...
[OLS *t* test]{style=color:pink;} Our (null) hypothesis states that $\beta_1$
