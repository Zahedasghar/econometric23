...
-   which tell us about the   [tendency of the estimator] if we took   [∞ samples], each with   [sample size] $\color{#e64173}{n}$.
...
[New question:]{style=color:blue;} <br>How does our estimator behave as our sample gets larger (as $n\rightarrow\infty$)?
...
[*probability limit*]{style=color:pink;} helps us analyze   [the asymptotic distribution of an estimator] (the distribution of the estimator as $n$ gets "big"<sup>.pink[†]</sup>).


 Here, "big" $n$ means $n\rightarrow\infty$. That's *really* big data.
...
*Recall.sub[1]:*   [Omitted-variable bias] occurs when we omit a variable in our linear regression model (_e.g._, leavining out $x_2$) such that
...
*Recall.sub[2]:* We defined the   [bias] of an estimator $W$ for parameter $\theta$
...
1. The omitted variable   [affects our outcome], _i.e._, $\beta_2 \neq 0$.
